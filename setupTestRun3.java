...
public class setupTestRun3 extends ExcelUtilities{
	...
	int runnerTSRowCount;
	arrayStructure s = new arrayStructure();
	
	@Before
	public void myBefore() throws Exception{
		xlPath = "C:\\Users\\A774\\AnyAut\\AnyAutV1\\src\\ExcelFiles\\TestSetup_6.xls";
	    xlRes = "C:\\Users\\A774\\AnyAut\\AnyAutV1\\src\\ExcelFiles\\RunSetup_6.xls";
					...
					if ((vModuleID.equals(vTCModuleID)) && (vTCExecute.equals("Y"))){
						...
						for(int j = 1; j< xRows_TS; j++){
							...
							if (vTCID.equals(vTS_TCID)){		
								vStepDetail = xlTS[j][3];
								vKW =	xlTS[j][4];
								vKWType = xlTS[j][5];
								vEID =	xlTS[j][6];
								vTD =	xlTS[j][7];									
								System.out.println("KW is : " + vKW);
								if(vKWType.equals("HLK")==true){
									String vHLK_KW = vKW;
									...
									for (int l = 1; l < xRows_HLK; l++){
										...
										if (vHLK_KW.equals(vHLK)==true){
											System.out.println("HLK executing : " + a);
											runnerTSRowCount++;
											vStepDetail =	xlHLK[l][1];
											vKW =	xlHLK[l][2];
											vEID =	getEID(xlHLK[l][3]);
											vTD =	getTD(xlHLK[l][4]);	
	...
	public void updateTestRunnerArray(int fA, int fJ){
		...
		*/
		System.out.println("Runner TS Row Count: " + runnerTSRowCount);
		
		s.add(fA, 0, vModuleID);										
		s.add(fA, 1, vTCID);
		s.add(fA, 2, xlTS[fJ][1] + "_Step_" + runnerTSRowCount);										
		s.add(fA, 3, vStepDetail);
		s.add(fA, 4, vKW);
		s.add(fA, 5, vEID);
		s.add(fA, 6, vTD);
		
		s.add(fA, 7, xlTS[fJ][8]); // Output
		s.add(fA, 8, xlTS[fJ][9]); // Result
		s.add(fA, 9, xlTS[fJ][10]); // Error
		s.add(fA, 10, xlTS[fJ][11]); // Screenshot
		s.add(fA, 11, xlTS[fJ][12]); // Timetaken
